...
import { KebabIcon, CopyIcon, TrashIcon, PencilIcon } from "@/components/icons";
...
type ModalMode = "duplicate" | "edit" | null;
...
export function ProposalActionsMenu({
  ...
  /** "refresh" re-fetches the current route (dashboard/activity list); "redirect-home"
   * navigates to "/" instead (used from a proposal's own workspace page,
   * which can't just refresh itself after its data is gone). Plain strings so
   * this stays usable from a Server Component parent - a function prop can't
   * cross that boundary. */
  afterDelete?: "refresh" | "redirect-home";
  /** Same idea, for a successful "Edit Details" save. The workspace page
   * holds the proposal in local state (for live editing elsewhere on the
   * page), so a router.refresh() alone wouldn't update what's already
   * rendered there - "reload" forces a full reload in that context. */
  afterSave?: "refresh" | "reload";
}) {
  ...
  async function handleSaveDetails() {
    ...
    try {
      ...
      setModalMode(null);
      setBusy(false);
      if (afterSave === "reload") window.location.reload();
      else router.refresh();
    } catch (err) {
    ...
    <div className="relative" onClick={(e) => e.stopPropagation()}>
      ...
      {menuOpen && (
        <>
          ...
          <div className="absolute right-0 top-9 z-30 w-48 rounded-xl border border-black/[0.08] bg-white py-1 shadow-lg">
            <button
      ...
      {modalMode && (
        ...
        >
          ...
          >
            <h2 className="text-lg font-semibold mb-1">{isEdit ? "Edit Proposal Details" : "Duplicate Proposal"}</h2>
            <p className="text-sm text-gray-500 mb-5">
              {isEdit
                ? "Update the client, contact, or call details for this proposal. The generated content isn't affected."
                : `Reuses everything from "${proposal.company_name}" (scope, deliverables, timeline, pricing) for a new client. You can regenerate whichever sections need to change afterward.`}
            </p>
            ...
            <div className="flex items-center justify-end gap-2 mt-6">
              ...
              <button
                onClick={isEdit ? handleSaveDetails : handleDuplicate}
                disabled={
              ...
              >
                {isEdit ? (busy ? "Saving…" : "Save Changes") : busy ? "Duplicating…" : "Duplicate"}
              </button>
            </div>
          </div>
        </div>
